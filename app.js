...
function renderCards(){
 ...
 cards.innerHTML=list.map(p=>{
  ...
  return '<article class="card"><div class="card-top" style="background-image:url('+getDestinationImage(p.destinazioni,p.nome+" "+(p.descrizione||""))+')"><div class="avatar">'+initials(p.nome)+'</div><div class="verified">'+verified+'</div></div><div class="card-body"><h3>'+escapeHtml(p.nome)+'</h3><div class="meta">'+escapeHtml([p.comune,p.provincia,p.disponibile_online?"Online":null].filter(Boolean).join(" · ")||"Consulenza di viaggio")+'</div><div class="tags">'+tags.map(t=>'<span class="tag">'+escapeHtml(t)+'</span>').join("")+'</div><div class="card-actions"><a class="btn btn-ghost" href="profilo.html?slug='+encodeURIComponent(slug)+'">Vedi profilo</a><a class="btn btn-primary" href="richiedi-proposte.html?professionista='+encodeURIComponent(slug)+'">Richiedi consulenza</a></div></div></article>';
 }).join("");
